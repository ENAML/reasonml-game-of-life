...
let compute_size = (a, new_length) => {
  let new_size = a.resizer(~curr_size=a.size, ~old_length=a.length, ~new_length);
...
let map = (f, a) => make_array(Array.map(f, Array.sub(a.data, 0, a.length)), default_resizer^);

let mapi = (f, a) => make_array(Array.mapi(f, Array.sub(a.data, 0, a.length)), default_resizer^);

/* let to_list = (a) => fold_right(List.cons, a, []); */
