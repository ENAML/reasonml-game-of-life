...
let make = (order) => {heap: DynArray.make(0, Obj.magic(0)), indices: Hashtbl.create(32), order};
...
let is_heap = (h) => {
  let ord = h.order;
  let rec is_heap = (i) =>
    (! has_left(h, i) || ord(get(h, i), get(h, left(i))) && is_heap(left(i)))
    && (! has_right(h, i) || ord(get(h, i), get(h, right(i))) && is_heap(right(i)));
  is_heap(0);
};
...
module type HashedType = {type t; let equal: (t, t) => bool; let hash: t => int;};
...
module Make = (H: HashedType) => {
  ...
  type t = queue;
  let make = (order) => {heap: DynArray.make(0, Obj.magic(0)), indices: Tbl.create(32), order};
  let length = (h) => DynArray.length(h.heap);
  let is_empty = (h) => length(h) == 0;
  let get = (h) => DynArray.unsafe_get(h.heap);
  let set = (h, i, x) => {
